...
const DynamicLayout = memo(function DynamicLayout({ children }: DynamicLayoutProps) {
  const location = useLocation();
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  
  // Get navigation routes from configuration
      ...
      <nav className="relative bg-slate-900/80 backdrop-blur-xl border-b border-slate-700/50 sticky top-0 z-50 shadow-2xl shadow-slate-900/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-18">
            ...
            {isDetailPage && currentRoute && (
              <div className="hidden md:flex items-center">
                <div className="text-right">
                  <h2 className="text-lg font-semibold text-slate-100">{currentRoute.title}</h2>
                  <p className="text-sm text-slate-400">{currentRoute.description}</p>
                </div>
              </div>
            )}
