...
from services.failure_vflashtwo_service import fetch_failure_vflashtwo
...
@router.websocket("/ws/vflashtwo")
async def failure_vflashone_ws(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            if cached_data:
                ...
                print(f"🗃️ ดึงจาก DB lineId={line_id} (VFLASH2)")
                with SessionLocal() as db:
                    raw_data = fetch_failure_vflashtwo(failure_query_data, db)
                    serialized_data = [
